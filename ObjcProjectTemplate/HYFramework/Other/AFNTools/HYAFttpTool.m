...
+ (void)requestWithType:(RequestType)requestType
    ...
    if (requestType == RequestType_POST) {
        [self POSTWithPath:path params:params success:^(id responseObject) {
            success(responseObject);
        } failure:^(NSError *error) {
            failure(error);
        } dataType:type showHUD:isShow];
...
+ (void)uploadImage:(UIImage *)image
    ...
    [manager POST:url parameters:parmeter headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
      NSData *data = nil;
